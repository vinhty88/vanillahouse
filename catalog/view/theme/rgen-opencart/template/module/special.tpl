...
<div class="box special-prd" id="special-prd<?php echo $module; ?>">
	...
	<?php if($this->config->get('RGen_colprdView_special') != 'scroll'){ ?>
	...
	<script type="text/javascript">
		$('#special-prd<?php echo $module; ?>').find('.box-product').carouFredSel({
			prev: '#special-prd<?php echo $module; ?> .carousel-controls .prev',
			next: '#special-prd<?php echo $module; ?> .carousel-controls .next',
			scroll: 1,
			width:'100%',
			swipe:{onTouch: true},
			auto: false,
...
if ($this->config->get('RGen_SlideShow_Mod_status') == 1 && $this->config->get('RGen_SlideShow_Mod') == 'special' && $getRoute == 'common/home') { ?>
<div class="box special-prd slidshow-prd list-carousel" id="slidshow-box">
<?php }else { ?>
<div class="box special-prd list-carousel<?php if ($this->config->get('RGen_carouselArrow') == 'tr') { ?> arrow-tr<?php } ?>" id="special-prd<?php echo $module; ?>">
<?php } ?>
...
$(document).ready(function(){
	...
	var prdObj 	= 	$('#special-prd<?php echo $module; ?>');
	var chkW 	= 	$('.content-body').width();
	var chkHome = 	$('.special-prd').hasClass('slidshow-prd');
	
	/* PRODUCTS DISPLAY ON SLIDESHOW 
	*******************************************/
	<?php if ($this->config->get('RGen_SlideShow_Mod_status') == 1 && $this->config->get('RGen_SlideShow_Mod') == 'special' && $getRoute == 'common/home') { ?>

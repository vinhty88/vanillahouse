...
<div class="custom-footer column-<?php echo $Active_modules; ?>">
	...
	<div id="aboutus-content" class="column">
		...
		<?php }else{ ?>
			<h2 class="header-2"><?php echo $this->document->RGen_CustomFt[$this->config->get('config_language_id')]['ftAbout_title']; ?></h2>
			<div><?php echo htmlspecialchars_decode( $this->document->RGen_CustomFt[$this->config->get('config_language_id')]['ftAbout'], ENT_QUOTES ); ?></div>
		<?php } ?>
	</div>
	...
	<div id="facebook-block" class="column fan_box">
		...
		<div class="facebook-block">
			<fb:like-box 
				height="290" 
				profile_id="<?php echo $this->config->get('FT_fb_ID'); ?>" 
				...
				<?php }?>
				show_faces="true" 
				colorscheme="<?php echo $this->config->get('RGen_fbtheme'); ?>" 
				stream="false" 
				header="false">
			</fb:like-box>
		</div>
